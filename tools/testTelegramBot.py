...
if not '--help' in [x.lower() for x in sys.argv]:
	...
	print("Do You want to see JSON detailed version of messages or just simple notifications?")
	...
	try:
		...
		def send_welcome(message):
			print ("'{}' message received from chat id {}".format(message.text, message.chat.id))
			if DEBUG: print (message)
			bot.reply_to(message, "Hello, I'm ePiframe Telegram bot test tool.\nStart writing something and I will reply.")
		...
		@bot.message_handler(content_types=['document', 'audio', 'photo', 'voice', 'sticker', 'video_note'])
		def echo_data(message):
			print ("Data '{}' received from chat id {}".format(message.content_type, message.chat.id))
			if DEBUG: print (message)
