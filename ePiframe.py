...
if '--help' in [x.lower() for x in sys.argv]:
		...
		if totalNum == 0:
			...
			if filterNum == 0:
				...
				if photo[constants.PHOTOS_SOURCE_LABEL] == constants.GOOGLE_PHOTOS_SOURCE:
					...
					err, origwidth, origheight = convertman.get_image_size(config.get('convert_bin_path'), sourceFile, constants.FIRST_FRAME_GIF)
					err, imagetype = convertman.get_image_format(config.get('convert_bin_path'), sourceFile, constants.FIRST_FRAME_GIF)
					...
					ret = shutil.copy(sourceFile, filename)
	...
	if '--test-convert' in [x.lower() for x in sys.argv]:
		...
		convertman = convertmanager()
		err, width, height = convertman.get_image_size(config.get('convert_bin_path'), filename, constants.FIRST_FRAME_GIF)
