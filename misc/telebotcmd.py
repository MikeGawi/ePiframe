class telebotcmd:
	...
	LONGER2_MSG = "\nMultiplicators sum up if already set"
	...
	DESC = { 
		...
		LONGER_CMD.replace('/', '').replace(OPTION_IND, '') : 'Display current photo # times longer'
	}
	
	DESCRIPTIONS = '\n'.join('_{}_ - {}'.format(key, value) for key, value in DESC.items())